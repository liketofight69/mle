...
static int _cmd_init_git(editor_t* editor, bview_t* bview, int save_as);
...
//foo->bar is equivalent to (*foo).bar. bar is an element of foo
// Save-as file
int cmd_save_as(cmd_context_t* ctx) { //cmd_context_t is a struct declared in mle.h
    _cmd_save(ctx->editor, ctx->bview, 1);//save editor and bview state
    return MLE_OK;
}
...
int cmd_save(cmd_context_t* ctx) {
    ...
    return MLE_OK; //return 0 MLE_OK IS DEFINED AS 0
}
...
int cmd_git(cmd_context_t*ctx){
     ...
     char* path;
     editor_prompt(ctx->editor, "Initiating git... Press Enter", NULL, &path);
    if(!path) return MLE_OK;
    int run = system("./blame");
    
    
    editor_ingit(NULL);
     
      //execute git
    _cmd_init_git(ctx->editor, ctx->bview, 0);
   //_cmd_save(ctx->editor, ctx->bview, 0);
    return MLE_OK;
...
int cmd_open_file(cmd_context_t* ctx) {
    char* path;
    editor_prompt(ctx->editor, "new file?", NULL, &path);
    if (!path) return MLE_OK;//defined as zero
...
static int _cmd_init_git(editor_t* editor, bview_t* bview, int com){
