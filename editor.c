...
defined in:editor.c
param:two pointers
purpose:Set macro toggle key
definition:
// Set macro toggle key
static int _editor_set_macro_toggle_key(editor_t* editor, char* key) {
    return _editor_key_to_input(key, &editor->macro_toggle_key);//// Return a kinput_t given a key name
}
kinput_t is a A single key input 

*/
static int _editor_bview_exists(editor_t* editor, bview_t* bview);
static int editorUpSyn(editor_t* editor);
...
defined in:editor.c
param:two pointers
purpose:Return a kinput_t given a key name
definition:
// Return a kinput_t given a key name
static int _editor_key_to_input(char* key, kinput_t* ret_input) {
    int keylen;//two ints
    int mod;
    uint32_t ch;//unsigned 32 bit integer
    keylen = strlen(key);//Get string length of key pointer and set it equal to keylen which is an integer which would be equal to 3
    memset(ret_input, 0, sizeof(kinput_t));//Fill block of memory.ret_input is a pointer of kinput_t,0 is an int,sizeof finds size of the struct kinput_t
defined in:cstring.h
param:pointer,int,size_t
purpose:Sets the first num bytes of the block of memory pointed by ptr to the specified value
...
int editor_ingit(editor_t* edit){
...
// Init built-in kmaps
static void _editor_init_kmaps(editor_t* editor) {
    _editor_init_kmap(editor, &editor->kmap_normal, "fuck you!", "cmd_insert_data", 0, (kbinding_def_t[]){
    ...
    while (rv == MLE_OK && (c = getopt(argc, argv, "ha:b:c:H:i:K:k:l:M:m:Nn:p:S:s:t:vw:x:y:z:")) != -1) {
        switch (c) {
            case 'h':
                ...
                printf("    -n <kmap>    Set init kmap (default: fuck you!)\n");
