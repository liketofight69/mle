...
typedef void (*bview_listener_cb_t)(bview_t* bview, baction_t* action, void* udata); // defines a pointer-to-function type. functions return void
...
typedef struct cmd_context_s cmd_context_t; // Context for a single command invocation  typedef means you no longer have to write struct
...
int editor_ingit(editor_t* editor);
